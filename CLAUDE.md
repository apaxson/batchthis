...
- **A form that should open in a modal on the page that links to it** (instead of navigating) uses the shared `#formModal` in `base.html` + its logic in `cellar-ledger.js`: add `data-cl-form-modal data-modal-title="..."` to the link (keep its real `href` as the no-JavaScript fallback). The view answers a modal request (`X-Requested-With: XMLHttpRequest`) with just the form partial, and a successful POST with `{"saved": true}`, which reloads the page. References: Add addon / Add note / Add test - `views/main.py` `_save_batch_record(..., partial=...)` and `includes/_addon_form.html`, `_note_form.html`, `_test_form.html` (the full page includes the same partial). react-select pickers inside the form are mounted automatically (load `dist/model-select.js` on the page). Scripts inside the inserted form DON'T run: put per-form behavior in `cellar-ledger.js`, keyed off data attributes, and run it on `DOMContentLoaded` and on the `cl:content-loaded` event the modal fires after loading content (see the reading-form code). A second modal from inside the form (e.g. "Use refractometer", `data-cl-refractometer`) is a hand-off - the form modal hides without clearing, the other modal opens, and the form comes back when it closes - because Bootstrap 4 can't stack modals.
...
- **Units of Measurement:** Any measurement data should use batchthis.fields.DescriptiveQuantityField().  The default storage in models.py for the database should always convert to metric, but redisplay based on entered measurement.  See batchthis.models.Batch.size for an example.
  ...
  - **Weight-or-volume form inputs** (e.g. an addition's amount - even counted items like tablets have a weight or volume) use `batchthis.fields.AmountField`: same behavior, accepts mass or volume ("4 g", "0.5 oz", "5 ml"). Reference: `BatchAdditionForm.amount`. Both share `MeasurementField`; a new kind of measurement input should subclass it with its own `dimensions` and messages.
  - **Test readings** (`BatchTest.value`) use `batchthis.fields.ReadingField`, whose allowed units depend on the test type: the form checks it against `READING_SPECS` in models.py (sg - a bare number up to 1.199 counts as sg, above that or with bx/brix it's Brix, converted alcohol-corrected against the batch's starting gravity via `Utils.refractometerCorrection` with the Brix noted in the description; °F/°C with "F"/"C" shorthand; ppm or mg/L; g/L; pH with no unit). Charts and fault rules use `BatchTest.chart_value` (the type's standard unit), displays use `display_value`. Never convert sg to Brix with pint - the registry defines them 1:1 but the real relationship isn't linear; Brix goes through the refractometer tool.
