...
- **A form that should open in a modal on the page that links to it** (instead of navigating) uses the shared `#formModal` in `base.html` + its logic in `cellar-ledger.js`: add `data-cl-form-modal data-modal-title="..."` to the link (keep its real `href` as the no-JavaScript fallback). The view answers a modal request (`X-Requested-With: XMLHttpRequest`) with just the form partial, and a successful POST with `{"saved": true}`, which reloads the page. Reference: Add addon - `views/main.py` `_save_batch_record(..., partial=...)` and `includes/_addon_form.html`. react-select pickers inside the form are mounted automatically (load `dist/model-select.js` on the page).
