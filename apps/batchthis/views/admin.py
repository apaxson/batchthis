...
IMPORT_TEMPLATE = 'batchthis/import.html'


def _as_list(items) -> list:
    """xmltodict gives a lone repeated element (one <YEAST> in <YEASTS>) as a dict, not a list."""
    if items is None:
        return []
...
def importData(request):
    """
    Tools > Import: BeerSmith .xml uploads - a single recipe, or a bulk export of
    misc items (adjuncts/fermentables/yeasts) to use in recipes.
    """
    log.debug("importData: method=%s files=%s", request.method, list(request.FILES.keys()))
    if request.method != "POST":
        return render(request, IMPORT_TEMPLATE)
    if not request.FILES:
        log.error("importData: POST with no file")
...
def _run_import(request):
    ...
    if 'recipe_upload' in request.FILES.keys():
        ...
        return render(request, IMPORT_TEMPLATE, {'uploaded_file_url': recipe_file_url})
        ...
        if 'MISCS' in parsed.keys():
            ...
            adjunct_list = []
            for data in _as_list(parsed['MISCS']['MISC']):
                adjunct = A()
                adjunct.load(data)
        ...
        if 'FERMENTABLES' in parsed.keys():
            ...
            for ferm_data in _as_list(parsed['FERMENTABLES']['FERMENTABLE']):
                fermentable = F()
        ...
        if 'YEASTS' in parsed.keys():
            ...
            for yeast_data in _as_list(parsed['YEASTS']['YEAST']):
                yeast = Y()
        ...
        log.error("importData: unexpected upload field(s) %s", list(request.FILES.keys()))
        messages.error(request, "No file found - choose a BeerSmith .xml file to upload.")
        return render(request, IMPORT_TEMPLATE)

    if counts:
        log.info("importData: misc import %s", counts)
        messages.success(request, f"Imported '{import_file.name}': {_summarize_counts(counts)}")
    else:
        log.info("importData: '%s' had no adjuncts, fermentables or yeasts", import_file.name)
        messages.warning(request, f"'{import_file.name}' had no adjuncts, fermentables or yeasts to import.")
    return render(request, IMPORT_TEMPLATE)
